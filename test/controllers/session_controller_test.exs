defmodule TodoApi.SessionControllerTest do
  ...
  test "does not create resource and renders errors when password", %{conn: conn} do
    conn = post conn, session_path(conn, :create), user: Map.put(@valid_attrs, :password, "notright")
    assert json_response(conn, 401)["errors"] != %{}
  end

  test "does note create resource and renders errors when email is invalid", %{conn: conn} do
